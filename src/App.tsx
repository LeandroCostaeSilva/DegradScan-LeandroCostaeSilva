...
import { Search, Download, FileText, Loader2, Sparkles } from "lucide-react"
import { MoleculeLogo } from "./components/MoleculeLogo"
import { generateDegradationReport } from "./services/api"
...
function App() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const result = await generateDegradationReport(searchTerm)
      setReport(result)
    } catch (err) {
        ...
        <div className="text-center mt-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-800/30 rounded-full border border-slate-700/50 backdrop-blur-sm">
            ...
            <p className="text-sm text-slate-400">
              DegradScan - Desenvolvido para profissionais da área farmacêutica e química analítica
            </p>
          </div>
        </div>
      </div>
