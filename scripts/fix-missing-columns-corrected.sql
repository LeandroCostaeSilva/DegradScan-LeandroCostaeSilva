...
-- Verificar estrutura das tabelas (versão corrigida)
DO $$
DECLARE
    rec RECORD;
...
BEGIN
    ...
    -- Verificar se tabela search_history existe
    SELECT EXISTS (
    ...
    IF table_exists THEN
        RAISE NOTICE 'Tabela search_history: EXISTS';
        ...
        END LOOP;
    ELSE
        RAISE NOTICE 'Tabela search_history: NOT EXISTS';
    END IF;
    ...
    -- Verificar se tabela substances existe
    SELECT EXISTS (
        SELECT 1 FROM information_schema.tables 
    ...
    IF table_exists THEN
        RAISE NOTICE 'Tabela substances: EXISTS';
    ELSE
        RAISE NOTICE 'Tabela substances: NOT EXISTS';
    END IF;
    
    -- Contar registros nas tabelas principais
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'substances') THEN
        FOR rec IN SELECT COUNT(*) as count FROM substances LOOP
            RAISE NOTICE 'Total de substâncias: %', rec.count;
        END LOOP;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'search_history') THEN
        FOR rec IN SELECT COUNT(*) as count FROM search_history LOOP
            RAISE NOTICE 'Total de pesquisas: %', rec.count;
        END LOOP;
    END IF;
